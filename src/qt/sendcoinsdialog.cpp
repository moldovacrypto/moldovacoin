    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Moldovacoin address (e.g. MdCFgKce467n4hZzaV9Vis1Eq5m53KtKa)"));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
		...
		{ 
#if QT_VERSION < 0x050000 
        formatted.append(tr("<b>%1</b> in %4 blocks of %5 MDC each to %2 (%3)?").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount),
			Qt::escape(rcp.label),  
			rcp.address,  
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Moldovacoin address"));
        }
        else
        {
