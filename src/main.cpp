...
const string strMessageMagic = "Moldovacoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees) 
{
    ...
    else if (pindexBest->nHeight+1 < 750000)
    {
        nSubsidy = 15 * COIN;
        return nSubsidy + nFees;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindexBest->nHeight+1 < 10000)
    {
        nSubsidy = 0.01 * COIN;
        return nSubsidy + nFees;
    }

    if(pindexBest->nHeight+1 < 1500000)
    {
        nSubsidy = 2 * COIN;
        return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Moldovacoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "17th of February 2018, 19.20 Chisinau - Moldovacoin Genesis";
        CTransaction txNew;
        txNew.nTime = 1518895200;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1518895200;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4437038;
		if(fTestNet)
        {
        ...
        assert(block.hashMerkleRoot == uint256("0x94c3ab8709fe77cd60bc14b1b917a6008027161f56e44f56c3363b57661349fb"));
